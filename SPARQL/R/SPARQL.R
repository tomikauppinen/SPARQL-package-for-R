...
SPARQL <- function(url="http://localhost/", query="", update="", 
 ns=NULL, param="", extra=NULL, format="xml", curl_args=NULL, parser_args=NULL, encoding="UTF-8") {
  if (!is.null(extra)) {
    ...
    if (query != "") {
      ...
      if(format == 'xml') {
        tf <- do.call(getURL,append(list(url = paste(url, '?', param, '=', gsub('\\+','%2B',URLencode(query,reserved=TRUE)), extrastr, sep=""),
         httpheader = c(Accept="application/sparql-results+xml"), .encoding=encoding),
        curl_args))
        ...
        tf <- do.call(getURL,append(list(url=paste(url, '?', param, '=', gsub('\\+','%2B',URLencode(query,reserved=TRUE)), extrastr, sep=""), .encoding=encoding),
          curl_args))
        ...
        df <- dropNS(df,ns)
          ...
          tf <- do.call(getURL,
            append(list(url=paste(url, '?', param, '=', gsub('\\+','%2B',URLencode(query,reserved=TRUE)), extrastr, sep=""), .encoding=encoding),
            curl_args))
